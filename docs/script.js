...
// Ensure data is loaded before any action
document.addEventListener('DOMContentLoaded', async () => {
    await syncCourseData();
    // Re-initialize cards since they might need data-subject listeners
    setupCards();
});

function setupCards() {
    document.querySelectorAll('.card[data-subject]').forEach(card => {
        card.onclick = () => openSubject(card.getAttribute('data-subject'));
    });
}
// Global State
let currentState = {
...
// Event Listeners
// Initial setup handled by setupCards in syncCourseData
...
function startExam(subject, levelIndex) {
    ...
    // Enable schema diagram if schema_url is present
    levelData.game.showSchemaDiagram = !!levelData.schema_url;
            ...
            <div id="diagram-overlay-panel" class="floating-diagram-panel hidden">
                ...
                <div class="panel-body">
                    ...
                    <div class="image-viewport" id="diagram-viewport">
                        <img src="${courseData[currentState.subject].levels[currentState.levelIndex].schema_url || 'database_diagram.png'}" id="diagram-image" alt="Database Diagram">
                    </div>
                </div>
            </div>
